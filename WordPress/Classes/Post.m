...
- (void)setTermsFromObjects:(NSArray *)objects {
    ...
    //NSArray *ignoreNames = [NSArray arrayWithObjects:@"post_tag",@"post_format",nil];
    NSArray *ignoreTaxonomies = [NSArray arrayWithObjects:@"post_format",nil];
    ...
    for (Term *term in array) {
        ...
        if (results && (results.count > 0)) {
            if([[term valueForKey:@"taxonomy"] isEqualToString:@"post_tag"]){
                NSString *resultsString = [[[results allObjects] valueForKeyPath:@"name"] componentsJoinedByString:@","];
                if(tags.length > 0){
                    tags = [NSString stringWithFormat:@"%@,%@",tags,resultsString];
                } else {
                    tags = resultsString;
                }
            } else {
